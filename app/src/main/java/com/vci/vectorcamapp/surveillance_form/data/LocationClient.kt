package com.vci.vectorcamapp.surveillance_form.data
...
class LocationClient @Inject constructor(
    ...
    suspend fun getCurrentLocation(): Result<Location, SurveillanceFormError> =
        suspendCancellableCoroutine { continuation ->
            ...
            if (ContextCompat.checkSelfPermission(
                ...
                if (continuation.isActive && !continuation.isCompleted) {
                    continuation.resume(Result.Error(SurveillanceFormError.LOCATION_PERMISSION_DENIED))
                }
            }
    ...
    private fun requestFreshLocation(
        ...
        continuation: CancellableContinuation<Result<Location, SurveillanceFormError>>
    ) {
        if (ContextCompat.checkSelfPermission(
                context, Manifest.permission.ACCESS_FINE_LOCATION
            ) == PackageManager.PERMISSION_GRANTED
            ...
            fusedLocationProviderClient.getCurrentLocation(
                ...
                when {
                    location != null -> {
                        ...
                        if (continuation.isActive && !continuation.isCompleted) {
                            continuation.resume(Result.Error(SurveillanceFormError.UNKNOWN_ERROR))
                        }
                    }
                }
